...
zz <- gzfile("df_departments.rds", "w")
saveRDS(df1, file = "data/raw_data/df_departments.rds")
...
df_department <- df1 %>% 
  ...
  separate(Period, c("month", "year"), extra='drop', remove= FALSE ) %>%
  ...
  subset(select = c("business", "year", "n"))
```
